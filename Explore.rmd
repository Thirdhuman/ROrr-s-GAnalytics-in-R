...
ggplot(data=sad_audience, aes(x=title, y=avg_MinPerWord, fill = "#FF6666")) +
...
unique(df_final$tags)
# Line Chart

housing<-c('Housing','housing','land','zoning','hud')
women<-c('leave','maternity','women','equal')

df_final$VBC_Issues=ifelse(grepl(paste(housing, collapse = "|")
																												,df_final$tags,fixed=F)==T,"Housing",ifelse(grepl(paste(women, collapse = "|")
																												,df_final$tags,fixed=F)==T,"Women's Issues",ifelse(df_final$tags!=0,"Other",NA)))
...
total_line %>%
ggplot(aes(x=obs_day,y=sessions
											,group=VBC_Issues
											,colour=VBC_Issues
)) + 
	geom_line()  + 
