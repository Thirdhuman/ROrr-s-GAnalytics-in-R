...
media_type<-media_type %>% distinct(author_full,title,sessions)
media_type=aggregate(sessions ~ author_full, media_type, sum)
ggplot(data=media_type, aes(x=reorder(author_full,sessions, function(x)sum(x)),y=sessions,colour=author_full) ) +
