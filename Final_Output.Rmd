...
analysis_identifier=initials("ALL",analysis_range)
library(ggrepel)
...
d=subset(d, freq >= 9000)
d$word=as.character(d$word)
d$word=ifelse(d$word=='cato','cato institute',d$word)
d$freq=ifelse(d$word=='cato institute',d$freq+200000,d$freq)
words=as.vector(d$word)
d$word=mclapply(words, toTitleCase, mc.preschedule = T)

#head(d, 10)
set.seed(12)
wordcloud(words = d$word, freq = d$freq, min.freq = 30, max.words=100,random.order = FALSE, scale=c(2.5,1), #res=300,
...
authur_views =subset(df_final, type != "Events" & type != 'Node')
authur_views<-authur_views %>% distinct(author_full,title,sessions)
# [10 'Jeffrey A. Singer`    
...
# (`Jeffrey A. Singer`,`Caroline Dorminey`, `Eric Gomez`, `John Samples`, `Emma Ashford`, `Daniel R. Pearson`, `Clark Neily`, `Roger Pilon`, `Corey A. DeAngelis`, `Chris Edwards`, `Dr. Steve Davies`, `Alan Reynolds`, `Vanessa Brown Calder` , `Simon Lester`, `Diego Zuluaga`, `Patrick J. Michaels`, `George Selgin`, `Michael D. Tanner`, `Johan Norberg`, `Neal McCluskey`, `Ryan Bourne`, `David Bier`, `Mustafa Akyol`, `Patrick G. Eddington` , `Jason Kuznicki`, `John Glaser`, `Dan Lips`, `Scott Lincicome`, `Alex Nowrasteh`, `Michael F. Cannon`, `Christopher A. Preble`,`A. Trevor Thrall`, `Ike Brannon`, `José Piñera`, `Ian Vásquez`, `Colin Grabow`, `Doug Bandow`, `David Boaz`, `Gabriel Latner`, `Emily Ekins`, `Jeffrey Miron`, `Juan Carlos Hidalgo`, `Tate Lacey`, `Randal O'Toole`, `Dick Komer`, `Trevor Burrus`, `Tom G. Palmer`, `Ilya Shapiro`, `Daniel J. Ikenson`, `Gene Healy`, `Walter Olson`, `Julian Sanchez`, `Aaron Ross Powell` , other_authur)

authur_views=aggregate(sessions ~ author_full, authur_views, sum)
ggplot(data=authur_views, aes(x=reorder(author_full,sessions, function(x)sum(x)),y=sessions,colour=author_full) ) +
	geom_bar(aes(fill = author_full),stat="identity",width=.4, position = "dodge")+coord_flip()+my_theme()+ 
	theme(legend.position="bottom",  legend.box = "horizontal")+theme(legend.title=element_blank()) +
...
Bar_type_df =subset(df_final, type != "events" & type != 'node') #& co_authors == "Caleb Brown")
																			 # Remove events
Bar_type_df$collaboration_yn=ifelse(Bar_type_df$author_full!=Bar_type_df$author_full & length(Bar_type_df$author_full)!=0, "Co-Authored", ifelse(Bar_type_df$author==Bar_type_df$author_full, "Sole Author",0))
